 ...
 *  Copyright (C) 2008 Olimex, dimitar at olimex.com
 *
...
static struct ads7846_platform_data ads_info = {
	...
	.y_min			= 100,
	.y_max			= 5300,
	.vref_delay_usecs	= 200,
	.settle_delay_usecs	= 250,	/* needed because of the capacitors */
	.penirq_recheck_delay_usecs = 100, /* additional nouse reduction */
	.x_plate_ohms		= (900+200)/2,
	.y_plate_ohms		= (900+200)/2,
	.pressure_max		= 15000,
	.debounce_max		= 5,
	.debounce_rep		= 1,
	.debounce_tol		= 100,
...
static struct at73c213_board_info at73c213_data = {
	.ssc_id		= 1,
	.shortname	= "SAM9-L9261 external DAC",
};
...
static struct spi_board_info ek_spi_devices[] = {
...
#if defined(CONFIG_TOUCHSCREEN_ADS7846) || defined(CONFIG_TOUCHSCREEN_ADS7846_MODULE)
	{
		.modalias	= "ads7846",
		.chip_select	= 2,
		.max_speed_hz	= 500 * 1000,	/* max sample rate * clocks per sample */
		.bus_num	= 0,
...
static struct fb_videomode at91_tft_vga_modes[] = {
	{
	        .name           = "GFT035AA320240 @ 50",
		.refresh	= 50,
		.xres		= 320,		.yres		= 240,
		.pixclock	= KHZ2PICOS(6000),

		.left_margin	= 20,		.right_margin	= 38,
		.upper_margin	= 4,		.lower_margin	= 15,
...
static int ek_gpio_ext_init(void)
{
	unsigned int i;
	int stat;

	/* export EXT connector to user space */
	for(i=0; i<ARRAY_SIZE(ext_conn_pins); ++i) {
		const unsigned int pin = ext_conn_pins[i].pin;
		const char *label = ext_conn_pins[i].label;

		stat = gpio_request(pin, label);
		if(!stat)
			stat = gpio_export(pin, 1);
		if(stat) {
...
static void __init ek_board_init(void)
{
	...
	at91_add_device_ssc(AT91SAM9261_ID_SSC1, ATMEL_SSC_TX | ATMEL_SSC_TD | ATMEL_SSC_TF);
#else
	/* MMC */
	at91_set_gpio_input(AT91_PIN_PC7, 1);	/* Card Present Input */
	at91_set_gpio_input(AT91_PIN_PC4, 1);	/* Write Protect Input */
...
MACHINE_START(SAM9_L9261, "Olimex SAM9-L9261")
	/* Maintainer: Olimex */
